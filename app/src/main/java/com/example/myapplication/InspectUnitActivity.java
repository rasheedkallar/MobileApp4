        ...
        SearchControl.addButton(Control.ACTION_SEARCH, view -> {
            ...
            Popup.setListener(new PopupSearch.PopupSearchListener() {
                ...
                public boolean onItemSelected(TableRow row, JSONObject data, DataService.Lookup lookup) {
                    ...
                    if(lookup != null){
                        new DataService(activity).postForList("InvItems[]", Select, "it0=> it0 = @0.InvItemUnits.Where(it1=> it1.Id = " + lookup.getId() + ").Select(it1=> it1.InvItem).FirstOrDefault()", null, array -> {
                            try {
                ...
                public void onTextChange(EditText editor, int keyCode) {
                    if(keyCode== 32){
                        ...
                        if(text != null && text.length() !=0){
                            ...
                            new DataService(activity).getList("MobileApi/Get?keyWords=" + text, array -> {
                ...
                if(s.length() >= start + after && start + after >1) {
                    ...
                    if(ascii == 10){
                        ...
                        if(barcode.length() != 0) {
                            ...
                            new DataService(activity).postForObject("MobileApi/GetItemByBarcode", rp, jsonObject -> {
                                Long unitId = null;
                                try {
